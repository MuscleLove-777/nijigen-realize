...
# コスプレ・アニコス系に関連するキーワード（タイトルやジャンルに含まれるべき語句）
RELEVANT_KEYWORDS = [
    "コスプレ", "アニコス", "メイド", "ナース", "制服",
    "セーラー", "スク水", "ブルマ", "バニー", "チャイナ",
    "ミニスカ", "ニーハイ", "レイヤー", "二次元",
]
...
def _is_relevant(product: dict, keyword: str) -> bool:
    """
    商品がテーマ（コスプレ・アニコス系）に関連するかチェックする
...
if __name__ == "__main__":
    # テスト実行
    products = fetch_products(keyword="コスプレ", hits=3)
    for p in products:
