...
CURRENT_SITE_URL = "https://musclelove-777.github.io/nijigen-realize/"
...
{{ actresses }}さんが出演するこの作品。コスプレ好きなら見逃せない一本です。
{% endif %}

{% if maker %}
> **{{ maker }}**からリリースされたこの作品は、コスプレジャンルで定評があります。
{% endif %}
...
INTRO_VARIATIONS = [
    "二次元が三次元に...！**「{title}」**は{genre_text}好きにはたまらない一本です。",
    "コスプレ好きにはたまらない...！**「{title}」**、{genre_text}好きなら絶対ハマる作品が来ました。",
    "{genre_text}で興奮したいならコレ！**「{title}」**がマジでおすすめ。",
    "話題沸騰中の**「{title}」**をピックアップ。{genre_text}のド直球コスプレ作品です。",
    "**「{title}」**が気になってる人、正解です。{genre_text}ジャンルの中でもガチで抜けるコスプレ作品。",
    "本日の厳選コスプレAVは**「{title}」**。{genre_text}好きの間で「神作品」と話題に。",
    "新着から見つけた掘り出し物！**「{title}」**、{genre_text}がたっぷり詰まった二次元実写化の一本です。",
    "サンプル動画だけでも興奮必至！**「{title}」**は{genre_text}の最高傑作かも。",
    "今週一番シコれるコスプレ作品はコレ。**「{title}」**、{genre_text}好きは見逃すな！",
    "MuscleLoveが厳選！**「{title}」**は{genre_text}好きなら見逃せないコスプレ作品です。",
    "MuscleLove編集部おすすめの**「{title}」**。{genre_text}ジャンルで今一番アツいコスプレ作品。",
    "MuscleLoveイチオシ！**「{title}」**、{genre_text}好きを唸らせる最高のコスプレ作品が来ました。",
]
...
def _generate_single_article(
    ...
    # タグの生成
    tag_list = genres[:5] if genres else ["コスプレ", "アニコス"]
    tags = ", ".join(f'"{t}"' for t in tag_list)

    # ジャンルテキスト（導入文用）
    genre_text = "・".join(genres[:3]) if genres else "コスプレ"
...
def _build_meta_description(title: str, genre_text: str, actresses: str) -> str:
    ...
    desc_variations = [
        f"{title}のサンプル動画・レビュー。コスプレ・アニコスの{genre_text}系作品を紹介。",
        f"コスプレ好き必見の「{title}」を徹底レビュー。{genre_text}好きにおすすめのコスプレAV。",
        f"再現度MAX！{genre_text}作品「{title}」。コスプレシチュエーションに興奮必至。",
        f"コスプレ作品の注目作「{title}」。{genre_text}のコスプレシーンをサンプル動画付きで紹介。",
    ]
    if actresses:
        desc_variations.append(
            f"{actresses}出演「{title}」。{genre_text}系コスプレ動画をチェック。"
        )
...
def _build_alt_text(title: str, actresses: str, genre_text: str) -> str:
    ...
    alt_variations = [
        f"コスプレ作品「{title}」のパッケージ画像",
        f"「{title}」{genre_text}系コスプレ作品のサムネイル",
        f"二次元が蘇る「{title}」の作品画像",
    ]
    if actresses:
        alt_variations.append(f"{actresses}出演「{title}」コスプレ作品の画像")
...
"""
    for idx, img_url in enumerate(images, 1):
        gallery_html += f'  <a href="{link_url}" target="_blank" rel="nofollow sponsored"><img src="{img_url}" alt="コスプレ作品のサンプル画像{idx}" style="width: 100%; border-radius: 4px;" loading="lazy" /></a>\n'
